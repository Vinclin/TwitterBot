...
public class TwitterBot {
     ...
     */
    private static final String PATH_TO_TWEETS = "files/dog_feelings_tweets.csv";
    /** Column in the PATH_TO_TWEETS CSV file to read tweets from */
    private static final int TWEET_COLUMN = 2;
     ...
     */
    private TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
     ...
     */
    static void writeStringsToFile(
     ...
     */
    String generateTweet(int numWords) {
        mc.reset();
        // Complete this method.
        String tweets = new String();

        if (0 == numWords || !mc.hasNext()) {
            return "";
        }
        if (0 > numWords) {
                ...
                while (count < numWords) {
                    if (mc.hasNext()) {
                        ...
                        if (!"<END>".equals(store)) {
                            ...
                            tweets = tweets + randomPunctuation();
                            mc.reset();
                        }
                    }
                }
     ...
     */
    private List<String> generateTweets(int numTweets, int numChars) {
     ...
     */
    private String generateTweetChars(int numChars) {
        if (0 > numChars) {
            throw new IllegalArgumentException(
        ...
        if (null == newTweet || newTweet.isEmpty()) {
            return "";
        }
     ...
     */
    private static int fixPunctuation(char punc) {
        return switch (punc) {
            case ';' -> 0;
            case '?' -> 1;
            case '!' -> 2;
     ...
     */
    void fixDistribution(List<String> tweet) {
