 ...
 * <p>
 * "an" is followed by "table" one time and "banana" twice, "table" is the end
...
public class MarkovChain implements Iterator<String> {
     ...
     *           MarkovChain
     */
    MarkovChain(NumberGenerator ng) {
        if (null == ng) {
    ...
    void addBigram(String first, String second) {
        ...
        if (null == first || null == second) {
            ...
            if (chain.containsKey(first)) {
                ...
                chain.put(first, chain.get(first));
            } else {
                ProbabilityDistribution<String> pd = new ProbabilityDistribution<>();
                pd.record(second);
                chain.put(first, pd);
            }
        }
     ...
     */
    void train(Iterator<String> sentence) {
        // Complete this method.
        if (null == sentence) {
            throw new IllegalArgumentException();
        }
    ...
    public String next() {
        ...
        String s = p;
        if (hasNext()) {
            ProbabilityDistribution<String> next = chain.get(s);
            if (null == next) {
                p = null;
            } else {
                p = next.pick(ng);
     ...
     */
    void fixDistribution(List<String> words, boolean pickFirst) {
        if (null == words || words.isEmpty()) {
            throw new IllegalArgumentException("Invalid word list for fixDistribution");
        }

        String curWord = words.remove(0);
        if (1 > startWords.count(curWord)) {
            throw new IllegalArgumentException(
                    "first word " + curWord + " " +
        ...
        while (!words.isEmpty()) {
            ...
            if (null != nextWord) {
                if (1 > curDistribution.count(nextWord)) {
                    throw new IllegalArgumentException(
                            "word " + nextWord +
