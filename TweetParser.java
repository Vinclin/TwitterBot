...
class TweetParser {
     ...
     * <p>
     * See <a href=
     * "https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern">...</a>
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        try {
            return csvLine.split(",")[csvColumn];
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (li.hasNext()) {
            String extract = extractColumn(li.next(), tweetColumn);
            if (null != extract) {
                data.add(extract);
            }
        }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        for (String s : sentence.split(" ")) {
            if (!s.matches(BAD_WORD_REGEX)) {
                String store = cleanWord(s);
                if (null != store) {
                    result.add(store);
                }
            }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        // Complete this method.
        List<String> cleaned = tweetSplit(removeURLs(tweet));
        List<List<String>> cleanTweets = new ArrayList<>();
        for (String s : cleaned) {
            List<String> store = parseAndCleanSentence(s);
            if (!store.isEmpty()) {
                cleanTweets.add(store);
            }
        }
        return cleanTweets;
    }
     ...
     */
    static List<List<String>> csvDataToTrainingData(
            BufferedReader br,
            int tweetColumn
    ) {
        // Complete this method
        List<String> cleaned = csvDataToTweets(br, tweetColumn);
        List<List<String>> data = new ArrayList<>();
        for (String s : cleaned) {
            data.addAll(parseAndCleanTweet(s));
        }
        return data;
    }
